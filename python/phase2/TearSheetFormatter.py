...
class TearSheetFormatter(object):
    ...
    @staticmethod
    def get_business_type(mpl, bt_tag):
        return mpl.business_types[bt_tag]
    ...
    def create_tearsheets(self, sheet_list, bt_tag, mpl, y_or_q, debug=False):
        for co_num, co_name in sheet_list.items():
            logger.error("%s %s", co_num, co_name)
            ...
            self.format_projections_section(co_num, co_name, bt_tag, Liquid_Assets_tag, mpl, y_or_q)
            self.format_projections_section(co_num, co_name, bt_tag, E07_tag, mpl, y_or_q)
    ...
    def build_df_for_quarterly_display(self, co, tag, period_type, y_or_q, b_idx, sz, debug=False):
        ...
        l1 = df.columns[0:split_idx-1]
        l2 = df.columns[split_idx:split_idx+3]
        cols = [val for pair in zip(l1, l2) for val in pair]
        df = df.reindex(columns=cols)
    ...
    def format_quarterly_section(self, co_num, co_name, bt_tag, mpl, tag, y_or_q, debug=False):
        ...
        business_obj = self.get_business_type(mpl, bt_tag)
        page = co_name
        ...
        df = self.build_df_for_quarterly_display_2(co_num, tag, quarterly_obj, y_or_q, 0,1, debug)
#        df = self.build_df_for_quarterly_display(co_num, tag, quarterly_obj, y_or_q, 0,1, debug)
        ...
        df = self.build_df_for_quarterly_display(co_num, tag, annual_obj, 0, 0,3, debug)
    ...
    def format_projections_section(self, co_num, co_name, bt_tag, tag, mpl, y_or_q):
        page = co_name
        ...
        proj_info_y = self.template_obj.get_projection_info(tag, bt_tag, YEARLY_IDX, DISPLAY_TS_PROJ)
        fids_with_spaces_y = proj_info_y[0]
        just_fids_y = filter(lambda a: a is not None, fids_with_spaces_y)
        just_fids_y = filter(lambda a: a != 'XXX', just_fids_y)

        yrly_proj_df = full_yrly_df.loc[just_fids_y]
        data_df = mpl.business_types[bt_tag].period_types[YEARLY_IDX].get_df_including_pcts(co_num, just_fids_y)
        data_df = data_df.iloc[:, 0:1]

        just_fids_q = just_fids_y

        if y_or_q == YEARLY_IDX:
            qtrly_proj_cube = mpl.qtrly_proj_dict[bt_tag]
            full_qtrly_df = qtrly_proj_cube.major_xs(co_num).transpose()
            qtrly_proj_df = full_qtrly_df.loc[just_fids_q]
            qtrly_proj_df = qtrly_proj_df.iloc[:,0:4]
            qtrly_proj_df.index = just_fids_y
        else:
            qtrly_proj_cube = mpl.business_types[bt_tag].period_types[QUARTERLY_IDX].ytd_cube
            proj_info_q = self.template_obj.get_projection_info(tag, bt_tag, QUARTERLY_IDX, DISPLAY_TS_PROJ)
            fids_with_spaces_q = proj_info_q[0]
            just_fids_q = filter(lambda a: a is not None, fids_with_spaces_q)
            just_fids_q = filter(lambda a: a != 'XXX', just_fids_q)

            full_qtrly_df = qtrly_proj_cube.major_xs(co_num).transpose()
            qtrly_proj_df = full_qtrly_df.loc[just_fids_q]
            qtrly_proj_df = qtrly_proj_df.iloc[:,0]
            qtrly_proj_df.index = just_fids_y


#        qtrly_proj_df = qtrly_proj_df.reindex(just_fids_q)
