...
class HealthBusinessType(BusinessType):
    ...
    def handle_remaining_files(self, data_dir, file_names):
        ...
        for file_name in file_names:
            if any(s in file_name for s in HEALTH_TEMPLATE_TAGS):
                ...
                try:
                    self.HEALTH_raw_df = pd.concat([self.HEALTH_raw_df, the_df], axis=1)
                except NameError:
                    self.HEALTH_raw_df = the_df
            else:
    ...
    def construct_data_cubes(self, template_wb):
        ...
        self.construct_select_data_cubes(template_wb, HEALTH_TEMPLATE_TAGS)
        return
